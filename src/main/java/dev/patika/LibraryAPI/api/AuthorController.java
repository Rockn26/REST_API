...
import dev.patika.LibraryAPI.dto.request.author.AuthorSaveRequest;
import dev.patika.LibraryAPI.dto.request.author.AuthorUpdateRequest;
...
public class AuthorController {
    ...
    public ResultData<AuthorResponse> update(@Valid @RequestBody AuthorUpdateRequest authorUpdateRequest) {
        Author authorUpdate = this.modelMapper.forRequest().map(authorUpdateRequest, Author.class);
        this.authorService.update(authorUpdate);
        return ResultHelper.success(this.modelMapper.forResponse().map(authorUpdate, AuthorResponse.class));
    }
    ...
    public ResultData<CursorResponse<AuthorResponse>> cursor(
        ...
        return ResultHelper.curse(authorResponsePage);
    }
